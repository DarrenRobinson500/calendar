...
import { getQuotes, createQuote, deleteQuote, reorderQuotes } from '../api.js'
...
export default function QuotesView() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null)
    try {
      const res = await createQuote({ text: text.trim(), author: author.trim() })
  ...
  const handleDrop = async (targetIdx) => {
    ...
    await reorderQuotes(reordered.map((e) => e.id))
  }

  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-xl font-semibold text-gray-800 mb-6">Quotes</h2>
          ...
          {entries.map((entry, idx) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-gray-800 text-sm whitespace-pre-wrap italic">"{entry.text}"</p>
                {entry.author && (
                  <p className="text-gray-600 text-sm mt-1">— {entry.author}</p>
                )}
